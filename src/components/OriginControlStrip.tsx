      ...
      <div className="mx-auto grid max-w-7xl md:grid-cols-2 md:px-8">
        ...
        {capabilities.map(({ title, body, href, brand }, index) => (
          <Link
            key={title}
            to={href}
            className={`group relative flex min-h-32 flex-col items-start gap-5 border-t border-slate-200 px-5 py-7 transition-colors duration-200 hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#EA6A12] sm:flex-row sm:px-8 md:border-t-0 ${index === 1 ? 'md:border-l' : ''}`}
          >
            ...
            <ArrowUpRight className="absolute right-5 top-7 h-4 w-4 text-slate-400 transition-all duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-[#EA6A12] sm:right-8" aria-hidden="true" />
          </Link>
        ))}
      </div>
    </section>
