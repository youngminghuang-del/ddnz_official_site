...
export default function ProductDiscoveryLinks({ source = 'home' }: { source?: 'home' | 'products' | 'insights' }) {
  ...
  const outdoorDescription = { en: 'Compare power stations, solar panels and vehicle fridges. Explore capacity, runtime and charging in the visual guide.', es: 'Compare estaciones, paneles solares y neveras. Explore capacidad, autonomía y carga en la guía visual.', ar: 'قارن محطات الطاقة والألواح الشمسية وثلاجات المركبات. استكشف السعة ومدة التشغيل والشحن في الدليل المرئي.' }[mobileLocale];
  const paths = [
    {name:copyFor(ui.hub,mobileLocale),description:copyFor(pages.cases.intro,mobileLocale),image:'/mobile-sourcing-media/case-colorways-ddnz-v1.webp',group:'mobile_accessories',links:[['/sourcing/mobile-accessories-from-china',copyFor(ui.hub,mobileLocale)],['/phone-cases',copyFor(ui.cases,mobileLocale)],['/phone-straps-charms',copyFor(ui.straps,mobileLocale)]].map(([to,label])=>[navigationPath(to,language),label])},
    { name: t[2], description: t[3], image: '/commercial-kitchen-media/kitchen-hero.webp', group: 'commercial_kitchen', links: [[kitchen + '#commercial-kitchen-equipment', t[6]], [kitchen + '#commercial-kitchen-benchmarks', t[7]], ...(language === 'en' ? [['/sourcing/commercial-ice-machines-from-china/', 'Ice machines'], ['/sourcing/commercial-electric-fryers-from-china/', 'Electric fryers'], ['/sourcing/commercial-electric-griddles-from-china/', 'Electric griddles']] : [])] },
