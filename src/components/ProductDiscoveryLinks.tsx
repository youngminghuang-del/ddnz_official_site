...
export default function ProductDiscoveryLinks({ source = 'home' }: { source?: 'home' | 'products' | 'insights' }) {
  ...
  const paths = [
    {name:copyFor(ui.hub,mobileLocale),description:copyFor(pages.cases.intro,mobileLocale),image:'/mobile-sourcing-media/case-colorways-ddnz-v1.webp',group:'mobile_accessories',links:[['/phone-cases',copyFor(ui.cases,mobileLocale)],['/phone-straps-charms',copyFor(ui.straps,mobileLocale)]].map(([to,label])=>[navigationPath(to,language),label])},
