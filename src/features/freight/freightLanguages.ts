...
export const freightAlternates = (path: string) => [
  ...freightLanguages.map(locale => ({hrefLang: locale === 'zh' ? 'zh-CN' : locale, href: `https://www.ddnzglobal.com${freightLanguagePrefix(locale)}${path}`})),
