...
export default function SupplierComparison({locale='en',pageId='compare',onRequest=()=>{}}){
 ...
 return <section className="ms-section ms-supplier-review" id="supplier-comparison">
  ...
  <div className="ms-dimension-detail"><figure><img src="/images/product-showcase/mobile/phone-case-machine-proof-v1.webp" alt={c('processPhoto')} width="800" height="600" loading="lazy"/><figcaption>{c('processPhoto')}</figcaption></figure><div aria-live="polite"><h3>{copyFor(dimension.name,locale)}</h3><p className="ms-eyebrow">{c('compare')}</p><p>{copyFor(dimension.compare,locale)}</p><p className="ms-eyebrow">{c('verify')}</p><p>{copyFor(dimension.verify,locale)}</p><a href="#buying-brief" className="ms-button" onClick={()=>onRequest('factory')}>{c('request')} ↗</a></div></div>
