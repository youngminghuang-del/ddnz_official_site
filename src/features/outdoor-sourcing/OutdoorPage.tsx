...
export default function OutdoorPage({guide=false}:{guide?:boolean}){
 ...
 const locale=productRouteParts(location.pathname).locale as 'en'|'zh'|'es'|'ar'|'ru'|'fr'|'pt'|'tr',meta=guide?powerGuideMetadata(locale):outdoorMetadata(locale),[legal,setLegal]=useState<LegalType>(null);
