...
export function outdoorMetadata(locale='en'){
 ...
 return {path,title,description,image,schema:{'@context':'https://schema.org','@graph':[
  ...
  {'@type':'BreadcrumbList',itemListElement:[{name:copy(ui.home,locale),item:base+(locale==='en'?'/':`/${locale}/`)},{name:copy(ui.products,locale),item:base+'/products/'},{name:copy(ui.heading,locale),item:base+path}].map((item,i)=>({'@type':'ListItem',position:i+1,...item}))},
...
export function powerGuideMetadata(locale='en'){
 ...
 return {path,title,description,image,schema:{'@context':'https://schema.org','@graph':[
  ...
  {'@type':'BreadcrumbList',itemListElement:[{name:copy(ui.home,locale),item:base+(locale==='en'?'/':`/${locale}/`)},{name:copy(ui.heading,locale),item:base+localizedProductPath(OUTDOOR_PATH,locale)},{name:copy(guideUI.eyebrow,locale),item:base+path}].map((item,i)=>({'@type':'ListItem',position:i+1,...item}))},
 ]}};
}
