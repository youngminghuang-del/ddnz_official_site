...
import { buildMobilePayload } from '../mobile-sourcing/buying.mjs';
import { FACTORY_CLIPS, VIDEO_COPY } from './production.mjs';
import {EN,t} from './locales/en.mjs';
import {PRODUCTS,BASIS,preset,economics,cartonFacts,money,number,referenceDate,names,estimate,packingLine} from './model.mjs';
import { ROUTES, pageForPath } from './routes.mjs';
import { DRAFT_KEY } from './handoff.mjs';
...
const key=DRAFT_KEY;
let state={rows:[],packing:'product',charging:'shipment',route:'sea',price:'',monthly:'',feeRate:'',requests:[]};
...
function refreshBrief(){
  try { const draft=readMixedDraft(sessionStorage); state.requests=draft.filmChecks; brief=(draft.rows.length||draft.filmChecks.length)?buildMobilePayload(draft,'en').message:'Your request is empty. Choose products or open the combined request to add phone cases and screen protectors.';
    $('#quote-text').value=brief; $('#copy-quote').disabled=!(draft.rows.length||draft.filmChecks.length); $('#download-quote').disabled=!(draft.rows.length||draft.filmChecks.length);
  } catch { $('#handoff-status').textContent='Your browser cannot restore the request. Open the combined request and check your selection.'; }
}
...
onRoot('click',e=>{
  ...
  const add=e.target.closest('[data-product-add]');if(add){try{addFilm(sessionStorage,add.dataset.productAdd);report('select_configuration');go(mixedBriefPath('en'));}catch{add.insertAdjacentHTML('afterend','<p role="alert">Unable to save the selection. Check browser storage or the 100-line limit.</p>');}}
...
all('[data-check]').forEach(el=>on(el,'change',()=>{state.requests=el.checked?[...new Set([...state.requests,el.dataset.check])]:state.requests.filter(x=>x!==el.dataset.check);try{const d=readMixedDraft(sessionStorage);d.filmChecks=state.requests;saveMixedDraft(sessionStorage,d);}catch{storage=false;}update();report('specification_change');}));
on($('#quote-entry'),'click',e=>{if(!isPlainAnchorClick(e,e.currentTarget))return;if(!result.valid){e.preventDefault();$('#input-errors').scrollIntoView({block:'center'});return;}try{importCalculator(sessionStorage,state);refreshBrief();}catch{e.preventDefault();$('#draft-state').textContent='Unable to save. Copy your selection before leaving.';}});
on($('#copy-quote'),'click',async()=>{if($('#copy-quote').disabled)return;try{await navigator.clipboard.writeText(brief);if(lifecycle.signal.aborted)return;$('#copy-status').textContent=t('quote.copied');report('copy_brief');}catch{if(lifecycle.signal.aborted)return;$('#quote-text').focus();$('#quote-text').select();$('#copy-status').textContent=t('quote.fallback');}});
on($('#download-quote'),'click',()=>{if($('#download-quote').disabled)return;const url=URL.createObjectURL(new Blob(['\ufeff'+brief],{type:'text/plain;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='DDNZ-screen-protector-brief.txt';a.click();setTimeout(()=>URL.revokeObjectURL(url),10000);$('#copy-status').textContent=t('quote.downloaded');report('download_brief');});
