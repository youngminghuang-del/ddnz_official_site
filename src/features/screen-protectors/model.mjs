...
export function makeBrief(result,state){
  ...
  return [t('quote.heading'),t('quote.reference',{date:referenceDate}),t('quote.destination'),' ',
    ...
    ...result.groups.map(g=>`${names[g.product]}${state.packing==='model'?' / '+g.models.join(', '):''}: `+packingLine(g)+'; '+t('calc.groupMeta',{dimensions:PRODUCTS[g.product].cartonCm.join(' × '),volume:number(g.cbm,5),weight:number(g.actualKg,3)})),
    ...result.warnings,t('quote.airBilling',{charging:t('calc.chargingOptions.'+state.charging)}),
    ...
    t('quote.checks'),...EN.quote.confirm.map((x,i)=>`${i+1}. ${x}`),
    ...state.requests.filter(id=>Object.hasOwn(EN.requests,id)).map(id=>'• '+EN.requests[id]),'',t('quote.final')].join('\n');
}
