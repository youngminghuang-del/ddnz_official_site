...
const CENTRAL_ASIA_DATA = {
  kazakhstan: {
    ...
    ru: {
      seoTitle: "Доставка из Китая под ключ | Карго и логистика в Казахстан и Узбекистан — DDNZ",
      seoDesc: "Надежная доставка из Китая под ключ от DDNZ Global. Собственный склад в Гуанчжоу, бесплатная консолидация, автодоставка и контейнерные перевозки сборных грузов.",
      headline: "Доставка из Китая под ключ",
      subheadline: "Профессиональные решения для автодоставки и контейнерных перевозок в Казахстан, Узбекистан и страны Центральной Азии. Снижайте риски импорта вместе с экспертами с 29-летним опытом.",
      transitWindow: "⏱️ Сроки доставки в Казахстан",
      transitDays: "15 - 25 дней",
      complianceRowTitle: "Аудит и сертификация ТР ТС ЕАЭС",
      complianceRowVal: "2 - 3 дня",
      ...
      solutions: [
        {
          ...
          desc: "Являясь членом Евразийского экономического союза (ЕАЭС), Казахстан имеет единую систему таможенных пошлин. Для регулируемых товаров DDNZ помогает импортерам заранее получить Декларацию о соответствии и классифицировать коды ТН ВЭД, избегая штрафов.",
          icon: "ShieldCheck"
        },
        {
          title: "Перегрузка на широкую колею 1520 мм",
          desc: "Колея в Китае составляет 1435 мм, а в Центральной Азии — 1520 мм. Груз должен быть перегружен на пограничных пунктах пропуска (Хоргос/Алашанькоу). Наша собственная команда на месте гарантирует быструю перегрузку.",
          icon: "Scale"
        },
        {
          title: "Стратегия двойного документооборота",
          desc: "Мы обеспечиваем строгое соответствие документов. Данные получателя в накладной должны на 100% совпадать с таможенными документами во избежание высоких плат за сверхнормативное хранение на складе.",
          icon: "FileText"
        },
        {
          title: "Защита прибыли на основе ТН ВЭД",
          desc: "Предупреждение! Между Китаем и ЕАЭС нет соглашения о свободной торговле со снижением пошлин. Сертификаты происхождения только подтверждают происхождение; DDNZ рассчитывает затраты по ставкам ТН ВЭД.",
          icon: "Search"
        }
      ],
      multimodalTable: [
        {
          mode: "✈️ Авиадоставка (Экспресс)",
          days: "3 - 7 дней",
        ...
        },
        {
          ...
          sellingPoint: "Регулярные поезда из хабов Китая (Сиань, Ляньюньган) в Алматы за 6-14 дней. Экономично для больших объемов.",
          warning: "Колея в Китае 1435 мм, в Казахстане 1520 мм. Требуется перегрузка на границе (Хоргос), которая занимает 1-3 дня."
        },
        {
      ...
      faqs: [
        {
          ...
          a: "Так как Казахстан входит в ЕАЭС, товары должны соответствовать единым техрегламентам (ТР ТС). Коды ТН ВЭД должны быть классифицированы точно. DDNZ проводит предварительный аудит во избежание штрафов и задержек в Алматы."
        },
        {
          q: "Как перегрузка на колею 1520 мм влияет на время доставки?",
          a: "Все ж/д грузы из Китая должны быть перегружены на широкую колею на границе. В пиковый сезон это может вызвать задержки. Команда DDNZ на границе оперативно координирует перегрузку грузов."
        },
        {
  ...
  uzbekistan: {
    ...
    ru: {
      ...
      headline: "Доставка из Китая под ключ",
      subheadline: "Профессиональные решения для автодоставки и контейнерных перевозок в Казахстан, Узбекистан и страны Центральной Азии. Снижайте риски импорта вместе с экспертами с 29-летним опытом.",
      transitWindow: "⏱️ Сроки доставки в Узбекистан",
      transitDays: "18 - 28 дней",
      complianceRowTitle: "Содействие в сертификации GOST-UZ",
      complianceRowVal: "3 - 5 дней",
      solutionsTitle: "Специальные решения для доставки в Узбекистан",
      solutionsSubtitle: "Профессиональный обход двойного налогообложения по CIF, сложной сертификации и жестких требований к предварительному декларированию.",
      solutions: [
        {
          title: "Предварительное онлайн-декларирование (правило 1 часа)",
          desc: "Таможня Узбекистана перешла на электронную предотчетность. Данные должны быть отправлены минимум за 1 час до прибытия на авто/жд границу. Цифровой документооборот DDNZ исключает задержки.",
          icon: "ShieldCheck"
        },
        {
          title: "Оптимизация расчета пошлин по CIF",
          desc: "Пошлины и НДС в Узбекистане рассчитываются на основе стоимости CIF (товар + доставка + страховка). DDNZ помогает оптимизировать объем (Cube Optimization) на нашем складе в Гуанчжоу для снижения стоимости доставки.",
          icon: "Scale"
        },
        {
          title: "Сертификация GOST-UZ и контроль ТН ВЭД",
          desc: "Для оборудования и электроники строго необходим сертификат соответствия GOST-UZ. Ошибки в кодах ТН ВЭД ведут к риску возврата или задержания груза на таможне.",
          icon: "FileText"
        }
      ],
      multimodalTable: [
        {
          ...
          sellingPoint: "Прямые рейсы из Китая в аэропорт Ташкента (TAS). Предварительное оформление гарантирует быструю выдачу.",
        ...
        },
        {
          ...
          sellingPoint: "Контейнерные поезда из ж/д узлов Китая (Сиань, Ляньюньган) в Ташкент за 14-22 дня. Стабильные экономичные тарифы.",
      ...
      faqs: [
        {
          ...
          a: "Перевозчик обязан передать полные электронные данные о грузе минимум за 1 час до прибытия на авто/жд границу (за 2 часа для авиа). DDNZ подготавливает и отправляет эти данные заранее."
        },
        {
          q: "Как работает узбекская формула расчета пошлин от CIF?",
          a: "Пошлины и НДС рассчитываются от стоимости CIF (стоимость товара + фрахт + страховка). Сначала начисляется пошлина, а затем НДС 12% на сумму стоимости товара, пошлины и фрахта. Мы оптимизируем объем груза, чтобы снизить фрахт."
        },
        {
          q: "Обязателен ли сертификат GOST-UZ для всех отправлений?",
          a: "Он строго обязателен для бытовой техники, оборудования и химической продукции. Без сертификата груз может быть задержан или помещен в зону уничтожения. DDNZ оформляет сертификаты до отправки."
        }
      ]
    }
  }
...
const PAGE_LANG_DATA: Record<string, Record<string, any>> = {
  ...
  ru: {
    ...
    insureText: "⭐ 29+ лет опыта · Эксперты по ж/д и автоперевозкам Шелкового пути",
...
export default function ShippingCentralAsia() {
  ...
  const activeLang = language === 'zh' ? 'zh' : (language === 'ru' ? 'ru' : 'en');
