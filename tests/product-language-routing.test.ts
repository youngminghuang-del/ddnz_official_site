...
test('all eight navigation languages use only authored product URLs', () => {
  ...
  for (const language of languages) {
    for (const path of englishProductPaths) {
      const target = `${['es','ar'].includes(language) && translated.has(path) ? `/${language}` : ''}${path}/`;
...
test('legacy aliases reach the available translation with query/hash and requested locale intact', () => {
  for (const language of languages) {
    ...
    for (const [alias, target] of [
      ...
      const translatedPrefix = ['/commercial-kitchen','/mobile-accessories','/outdoor-products'].includes(alias) && ['es','ar'].includes(language) ? prefix : '';
...
test('SPA, reload and native product arrivals preserve preference; explicit English home resets it', () => {
  ...
  assert.equal(resolveNavigationLanguage(product, stored, switched), 'es'); // refresh
  ...
  assert.equal(resolveNavigationLanguage(product, stored), 'en'); // fresh product visit after EN home
...
test('authored language URLs take precedence over stale preferences and return to the matching English page', () => {
  ...
  assert.equal(navigationPath('/screen-protectors/videos/#cutting', 'ar'), '/screen-protectors/videos/#cutting');
  assert.equal(navigationPath('/ar/screen-protectors/private-label/', 'fr'), '/screen-protectors/private-label/');
...
test('malformed stored state cannot select a bogus locale or poison internal URLs', () => {
  for (const value of ['de', 'constructor', '__proto__', null, {}, 1]) {
    ...
    assert.equal(resolveNavigationLanguage('/products', 'tr', { navigationLanguage: value }), 'tr');
  }
