...
test('four distinct purchasing needs have complete equivalent copy and real media in all eight languages', () => {
  ...
  for (const locale of ['en','zh','es','ar','ru','fr','pt','tr']) {
    ...
    assert.deepEqual(Object.keys(copy).sort(), required.slice().sort());
    ...
    for (const guide of buyerGuides) {
      ...
      assert.deepEqual(Object.keys(page).sort(), Object.keys(source).sort());
      ...
      assert.ok(page.description.length >= 40); assert.ok(page.steps.every(step => step[0].length > 0 && step[1].length > (locale === 'zh' ? 25 : 80)));
...
test('buyer routes have reciprocal eight-language canonicals and correct breadcrumb parents', () => {
  for (const guide of buyerGuides) for (const locale of ['en','zh','es','ar','ru','fr','pt','tr']) {
    ...
    assert.equal(meta.path, route); assert.equal(schema['@graph'][0].inLanguage, locale === 'zh' ? 'zh-CN' : locale);
    const last = schema['@graph'][1].itemListElement.at(-1);
    assert.equal(schema['@graph'][1].itemListElement[0].item, `https://www.ddnzglobal.com${locale === 'en' ? '/' : locale === 'zh' ? '/zh-cn/' : `/${locale}/`}`);
    assert.equal(last.item, `https://www.ddnzglobal.com${route}`);
    assert.deepEqual(productAlternates(route).map(item => item.hrefLang), ['en','zh-cn','es','ar','ru','fr','pt','tr']);
  }
  assert.equal(buyerGuideForPath('/screen-protectors/private-label-copy/'), undefined);
  assert.equal(hasProductTranslation('/screen-protectors/calculator/', 'ar'), true);
