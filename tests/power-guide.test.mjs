...
test('guide SSR, assets, language alternates and catalogue links exist in three languages',()=>{
 for(const locale of ['en','es','ar']){const html=renderToStaticMarkup(React.createElement(Content,{locale}));assert.equal((html.match(/<h1>/g)||[]).length,1);assert.ok(html.includes('id="power-guide-model"'));assert.ok(html.includes('id="appliance-plan"'));assert.ok(html.includes('id="energy-flow"'));assert.ok(html.includes('id="power-charge-model"'));assert.ok(html.includes('id="power-plan-model"'));assert.doesNotMatch(html,/NaN|undefined|\[object Object\]|16 phone charges|renewal needed/);for(const [,src]of html.matchAll(/<img[^>]*src="([^"]+)"/g))assert.ok(fs.existsSync(new URL('../public'+src,import.meta.url)));assert.equal(powerGuideMetadata(locale).path,localizedProductPath(powerGuidePath,locale));assert.equal(productAlternates(powerGuidePath).length,3);}
});
