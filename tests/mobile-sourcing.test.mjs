...
test('supplier tiers switch at exact minimums and never quote samples or invalid quantities',()=>{
 assert.equal(mobileProducts.length,11);
 assert.equal(mobileProducts.filter(p=>p.tiers).length,7);
 for(const p of mobileProducts){
  ...
  assert.match(p.url,/^https:\/\/www\.alibaba\.com\/product-detail\/.+_\d+\.html$/);
 }
 assert.equal(referencePrice(productById('patch-trendcomm'),100),5.79);
 assert.equal(productById('patch-trendcomm').sample,33.97);
 assert.equal(referencePrice(productById('chain-trendcomm'),100),38.37);
...
test('cost model requires explicit costs, uses entered FX and shows losses as gross margin',()=>{
 ...
 let r=buyingScenario(base);assert.equal(r.status,'complete');assert.ok(Math.abs(r.unitCost-3.055)<1e-9);assert.ok(Math.abs(r.batchCost-305.5)<1e-9);assert.ok(Math.abs(r.grossMargin-38.9)<1e-9);
 r=buyingScenario({...base,sale:'3',tax:'0.50'});assert.ok(Math.abs(r.unitCost-3.555)<1e-9);assert.ok(r.grossMargin<0);
 r=buyingScenario({...base,currency:'CNY',rate:'',sale:'25'});assert.equal(r.unitCost,18.33);assert.ok(Math.abs(r.batchCost-1833)<1e-9);
...
test('draft restores only known products and preserves buyer data across languages',()=>{
 ...
 for(const locale of ['en','es','ar']){const p=buildMobilePayload(copy,locale);assert.ok(p.message.includes(productById('folio-bida').name[locale]));assert.ok(p.message.includes('Buyer device'));assert.ok(p.message.includes('Dubai'));assert.ok(p.message.includes('CNY 17.68'));assert.doesNotMatch(p.message,/JPY/);}
